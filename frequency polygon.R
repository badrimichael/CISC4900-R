...
ggplot(ggdf_0,aes(x=time,fill=as.factor(type), color=type)) +geom_freqpoly(bins=20) + theme_classic() + ggtitle("25000 total reward: 200 of each agent (0 percent)")




qplot_10 <- c()
qplot_10$group[1:200] <- "q-learning"
qplot_10$val <- q_learning_10
q_10<- cbind(qplot_10$group,qplot_10$val)

qvplot_10 <- c()
qvplot_10$group[1:200] <- "qv-learning"
qvplot_10$val <- qv_learning_10
qv_10<- cbind(qvplot_10$group,qvplot_10$val)

esarsaplot_10 <- c()
esarsaplot_10$group[1:200] <- "expected_sarsa"
esarsaplot_10$val <- expected_sarsa_10
sar1_10<-cbind(esarsaplot_10$group,esarsaplot_10$val)

sarsaplot_10 <- c()
sarsaplot_10$group[1:200] <- "sarsa"
...
ggplot(ggdf_10,aes(x=time,fill=as.factor(type), color=type)) + geom_freqpoly(bins=20) + theme_classic() + ggtitle("25000 total reward: 200 of each agent (10 percent)")
